...
class PERDQN:
    ...
    def __init__(self, env):
        ...
        env_dict = {'state': {'shape': self.env.observation_space.shape},
                    ...
                    'done': {'dtype': np.bool}}
        self.memory = cpprb.PrioritizedReplayBuffer(10**6, env_dict, alpha=ALPHA)
    ...
    def calculate_loss(self, batch):
        ...
        return torch.nn.functional.mse_loss(Qval, Qgoal, reduction='none')
    ...
    def train(self, num_steps, progress_prefix='', *args, **kwargs):
        ...
        for idx in progress_bar:
            ...
            # Train
            batch = self.memory.sample(BATCH_SIZE, beta=BETA)
            elementwise_loss = self.calculate_loss(batch)
            weights = torch.tensor(batch['weights']).to(device)
            self.update_model(torch.mean(elementwise_loss * weights))

            # Update PER weights
            self.memory.update_priorities(batch['indexes'], elementwise_loss.detach().cpu())
