...
class Network(torch.nn.Module):
    ...
    def forward(self, t):
        ...
        return self.value(t) + advantage - advantage.mean(-1, True)
...
class DDDQN:
    ...
    @property
    def eps(self):
        return self.eps_min + (self.eps_max - self.eps_min) * np.exp(-self.step_cnt * self.eps_dec)
    ...
    def update_model(self, loss):
        ...
        if self.step_cnt % TARGET_PERSISTENCE == 0:
            self.target.load_state_dict(self.policy.state_dict())
